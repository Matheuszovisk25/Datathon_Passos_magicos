...
def exibir_cassia():
    ...
    def grafico_teia(df_aluno, title):
        ordem_categorias = ['INDE', 'IAA', 'IEG', 'IPS', 'IDA', 'IPP', 'IPV', 'IAN']

        df_ordenado = df_aluno[ordem_categorias]

        fig = go.Figure()
        fig.add_trace(go.Scatterpolar(
        r=df_ordenado.iloc[0].values,  
        theta=ordem_categorias,
        fill='toself',
        fillcolor='rgba(255, 215, 0, 0.5)',  
        line=dict(color='orange', width=2),  
        marker=dict(color='orange', size=0),  
        name='Desempenho'
    ))

        for i, valor in enumerate(df_ordenado.iloc[0].values):
            fig.add_annotation(
            x=ordem_categorias[i],
            y=valor,
            text=f'{valor:.1f}',  
            showarrow=False,
        ...
        notas = ", ".join([f"{label}: {df_aluno[label].values[0]:.1f}" for label in ordem_categorias])
        fig.add_annotation(
        text=f"Notas: {notas}",
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Análise de INDE", "Distribuição de Idades", "Comparativo de INDE", "Gráfico de Teia"])
...
E a pedra Topazio indica que os alunos que recebem esta pedra seu desempenho geral baseado no indicador INDE, varia entre 8,2 a 9,3.""")
    ...
    with tab4:
        ...
        if df_aluno.empty:
            ...
            st.write(df_aluno)
            fig = grafico_teia(df_aluno, title=f'Performance do {aluno_selecionado} - ano {ano_selecionado}')
            st.plotly_chart(fig)
